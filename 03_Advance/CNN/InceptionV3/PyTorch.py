...
inceptionv3 = Build_InceptionV3(input_channel=imgs_tr.shape[-1], num_classes=5).to(device)
...
for epoch in range(epochs):
    ...
    for i, (batch_img, batch_lab) in enumerate(train_loader):
        ...
        y_pred = inceptionv3.forward(X)

        loss_total = criterion(y_pred[0], Y)
        loss_aux = criterion(y_pred[1], Y)

        loss = loss_total + 0.4*loss_aux
    ...
    with torch.no_grad():
        ...
        for (batch_img, batch_lab) in val_loader:
            X = batch_img.to(device)
            Y = batch_lab.to(device)
            y_pred = inceptionv3(X)
            val_loss_total = criterion(y_pred[0], Y)
            val_loss_aux = criterion(y_pred[1], Y)
            val_loss += val_loss_total + 0.4 * val_loss_aux
