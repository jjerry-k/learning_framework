...
class CNN(Model):
    ...
    def build_model(model):
        ...
        model.x, model.y = x, y # change line 57 to line 51 
        super().__init__(model.x, model.y) # change __init__ to build_model
        
        model.cl_part = Model(x, z_cl) # Model(input, output)
...
class DataSet():
    ...
    def add_channels(self):
        ...
        if len(X.shape) == 3: # if gray image
            # N : batch
            # img_rows : height
            # img_cols : width
            ...
            # check backend type 
            if K.image_dim_ordering() == 'th': # if theano
                # [N, height, width] -> [N, 1, height, width]
...
class Machine():
    def __init__(self, X, y, nb_classes=2, fig=True):
        ...
        self.set_data(X, y) # Setting data
        self.set_model() # Setting model
        self.fig = fig
